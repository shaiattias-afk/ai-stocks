"""filings/archive.py — the compressed SEC filing archive.

Ported from `scripts/161_filings_archive_core.py` (logic unchanged; only
`PROJECT_DIR` now comes from the package instead of being derived from the
script's own location). Every consumer imports this normally rather than
through `importlib.spec_from_file_location`.

Goal: stop downloading SEC files the pipeline never reads, and store the
files it DOES need inside DuckDB as gzip-compressed BLOBs with integrity
hashes, instead of ~106 loose files per filing on disk (measured: a locked
filing downloads ~106 files, the extraction pipeline reads ~7).
