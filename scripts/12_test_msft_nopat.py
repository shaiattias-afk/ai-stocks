...
def main() -> None:
    ...
    for as_of_date in AS_OF_DATES:
        ...
        if len(period_ends) != 1:
            raise RuntimeError(
                f"הנתונים אינם מאותה שנה עבור {as_of_date}:\n"
                f"Operating Income: {operating_row['period_end'].date()}\n"
                f"Pretax Income: {pretax_row['period_end'].date()}\n"
                f"Tax Expense: {tax_row['period_end'].date()}"
            )
        ...
        if not 0 <= effective_tax_rate <= 1:
            raise RuntimeError(
                ...
                f"עבור {as_of_date}: {effective_tax_rate:.2%}"
            )
        ...
        results.append(
            {
                ...
                "period_end": operating_row["period_end"].date(),
                "latest_filing_date": max(filing_dates).date(),
                ...
                "nopat_usd": nopat,
                "pretax_income_tag": pretax_row["tag"],
    ...
    print()
    print("=" * 145)
    print("Microsoft NOPAT point-in-time test")
    print("=" * 145)
    ...
    print()
    print(
        "הבדיקה עברה: NOPAT חושב לחמש השנים "
