"""
Production-grade, general (not ticker- or accession-specific), fail-closed
XBRL warehouse loader. Promotes the entry-point-detection and validation
logic already proven read-only-safe in
scripts/139_corrected_warehouse_loader_entry_point_detection.py (scratch
proof) into a module suitable for writing to the real production
warehouse database — used by scripts/145 for exactly one accession
(NVDA 0001045810-19-000079), but contains no logic specific to that
accession or to NVDA.

WHY THIS EXISTS (see TASK_141/scripts/139 for the full root-cause
history): the original warehouse loader
(scripts/121_quarterly_batch_runner.py's load_and_warehouse_one_10q())
always uses the SEC filing index's primaryDocument as the sole Arelle
entry point, and sets status="PASS" the instant Arelle's Session exits
without a Python exception -- never checking whether anything was
actually extracted. This silently produces a false PASS for any filing
whose primaryDocument is a plain (non-Inline) HTML page while the real
machine-readable facts live in a separate traditional-XBRL instance
document -- exactly NVDA's Q1 FY2020 10-Q, its only pre-Inline-XBRL
quarterly filing in this project's universe (confirmed by TASK_141's
185-accession global audit: the only such case anywhere).

Public API:
  find_locked_manifest(ticker, report_date, form="10-Q") -> dict
  detect_entry_point(locked_dir, primary_document_path) -> dict
  run_production_warehouse_load(ticker, report_date, warehouse_db_path,
                                 form="10-Q") -> dict

Entry-point detection, PASS requirements, and failure categories are all
general -- no ticker or accession name appears anywhere in this module's
logic (only as data flowing through parameters).
"""
