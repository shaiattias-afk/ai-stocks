...
#
# Consolidates the logic already proven separately in scripts 37c-43 into
...
#                                  (e.g. Free Cash Flow), never their own
#                                  XBRL row search.
#
...
#
# v3 (this file, 42/43 unmodified): adds Operating Cash Flow and Capital
# Expenditures as two new BUILT_IN_METRICS on the Cash Flow statement
# (a different primary-statement role family than revenue/net income/
# operating income), and Free Cash Flow as a DERIVED_METRICS entry
# (operating_cash_flow - capex), computed after extraction rather than
# searched for as its own presentation row.
# =============================================================================
...
# prior proofs (scripts 37c through 43).
...
class MetricDefinition:
    ...
    # (derived figures, or non-cash/disclosure look-alikes — see capex).
...
BUILT_IN_METRICS: dict[str, MetricDefinition] = {
    ...
    "operating_cash_flow": MetricDefinition(
        ...
        # Cash Flow statements are a separate primary-statement family
        # from the income statement family used above — a universal SEC
        # statement type, not specific to any issuer.
    ...
    "capex": MetricDefinition(
        ...
        role_include_pattern=r"cash\s*flows?",
        role_exclude_pattern=None,
        # Three unrelated common phrasings for the same
        # us-gaap:PaymentsToAcquirePropertyPlantAndEquipment concept —
        # "Capital expenditures" (e.g. Oracle), "Additions to property
        # and equipment" (e.g. Microsoft), "Purchases of property and
        # equipment" (e.g. Meta). None of these words are shared across
        # all three, so the mention pattern must OR them explicitly.
        mention_pattern=(
            r"capital\s+expenditures?|"
            r"(?:additions|purchases)\s+(?:to|of)\s+property"
        ),
        # A same-statement, non-cash disclosure trap: "Unpaid capital
        # expenditures" / "...incurred but not yet paid" / "...in
        # accounts payable" report the us-gaap:CapitalExpendituresIncurred
        # ButNotYetPaid concept, not the cash CapEx payment, but their
        # labels can still contain "capital expenditures" or "property
        # and equipment" — excluded explicitly so they never create a
...
class DerivedMetricDefinition:
    name: str
    # Names of BUILT_IN_METRICS this value is computed from. Both must
    # PASS, over the same reporting period, for this to compute.
    component_metrics: tuple[str, str]
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "derived metrics computed from them, from an already locked "
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v3_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v3_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v3_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v3_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v3_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v3_orchestration.log"
        ),
    }
...
# convention already established in this project (see msft_fcf_test.csv),
# distinct from the single-metric "{ticker}_{metric}_test.csv" convention
# (see msft_revenue_test.csv). Both are checked; neither is required.
...
def identify_canonical_row(
    ...
    1. STATEMENT ROLE IDENTIFICATION — restrict to a top-level
       ...
       "Comprehensive Income" statement for income-statement metrics; not
       used for cash-flow metrics, which have no such variant).
    2. CANONICAL ROW IDENTIFICATION — within that role, collect
       non-abstract rows whose label matches `mention_pattern` and does
       not match `exclude_label_pattern` (e.g. excluding non-cash
       "incurred but not yet paid" CapEx disclosure look-alikes), then
       apply a two-tier decision: Tier A (`attributable_pattern`) —
       preferred when it resolves to exactly one row; Tier B
       (`plain_pattern`) — a single unqualified line-item label
       otherwise.
    ...
    with TargetRowNotFound (→ REVIEW_REQUIRED), never a guess.
    """
...
# =============================================================================
# 8. DERIVED METRICS — computed from already-extracted built-in metrics.
#    No presentation row search of their own; full lineage back to both
#    component facts is retained.
# =============================================================================
...
def compute_derived_metric(
    ...
    ocf_result, capex_result = (
        components[component_names[0]],
        components[component_names[1]],
    )

    periods_match = (
        ocf_result.get("period_start") == capex_result.get("period_start")
        and ocf_result.get("period_end") == capex_result.get("period_end")
    )
    ...
    if not periods_match:
        return {
            ...
            "error": (
                ...
                "אוטומטי.\n"
                f"{component_names[0]}: {ocf_result.get('period_start')} "
                f"- {ocf_result.get('period_end')}\n"
                f"{component_names[1]}: "
                f"{capex_result.get('period_start')} - "
                f"{capex_result.get('period_end')}"
            ),
            "is_derived_metric": True,
            "formula": definition.formula_description,
            "value": None,
    ...
    value = ocf_result.get("value") - capex_result.get("value")
    ...
    return {
        ...
        "formula": definition.formula_description,
        "value": value,
        "unit": ocf_result.get("unit"),
        "period_start": ocf_result.get("period_start"),
        "period_end": ocf_result.get("period_end"),
        "components": component_lineage,
    }
...
def run_engine(
    ...
    requested_built_in = [
        name for name in metric_names if name in BUILT_IN_METRICS
    ]
    requested_derived = [
        name for name in metric_names if name in DERIVED_METRICS
    ]

    # Derived metrics need their component built-in metrics extracted
    # even if the caller did not ask for them explicitly, so the
    # underlying source facts (with full lineage) are always available.
    required_built_in = list(requested_built_in)

    for derived_name in requested_derived:
        for component_name in DERIVED_METRICS[derived_name].component_metrics:
            if component_name not in required_built_in:
                required_built_in.append(component_name)

    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
        paths["orchestration_log_file"],
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v3 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
