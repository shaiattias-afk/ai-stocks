...
# revenue extraction proofs (scripts 37c/37d/38/39) and on the first
# net-income attempt (script 40).
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_net_income_v2_arelle_presentation.csv"
        ),
        "candidates_csv": (
            DATA_DIR / f"{prefix}_net_income_v2_fact_candidates.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_net_income_v2_row_candidates.csv"
        ),
        "result_file": (
            DATA_DIR / f"{prefix}_net_income_v2_fact_result.json"
        ),
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_net_income_v2_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_net_income_v2_orchestration.log"
        ),
    }
...
def select_target_row(
    ...
    1. Restrict to the primary Statement-type role: title contains
       "operations" (e.g. "STATEMENTS OF OPERATIONS"), or contains
       "income" but NOT "comprehensive" (e.g. "INCOME STATEMENTS", while
       excluding a separate "COMPREHENSIVE INCOME STATEMENT" role — a
       distinct, universally recognized statement type across SEC filers
       that also happens to start from the same net-income line, which
       caused a genuine two-candidate ambiguity in the first attempt,
       script 40, on Oracle).
    ...
    is_income_role = presentation["role_definition"].str.contains(
        r"income",
        case=False,
        regex=True,
        na=False,
    ...
    is_comprehensive_role = presentation["role_definition"].str.contains(
        r"comprehensive",
        case=False,
        regex=True,
        na=False,
    ...
    is_primary_statement_role = (
        presentation["role_definition"].str.match(
            r"^\d+\s*-\s*Statement\s*-",
            na=False,
        )
        & (is_operations_statement | (is_income_role & ~is_comprehensive_role))
    )
    ...
    if base_candidates.empty:
        raise TargetRowNotFound(
            "לא נמצאה אף שורת Net Income בתוך Statement role ראשי של "
            "Income/Operations (לא כולל Comprehensive Income)."
        )
...
def run_extraction(ticker: str, report_date: str) -> dict[str, object]:
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — NET INCOME FACT EXTRACTION v2 "
