"""
Generalizes scripts/36b_download_accession_locked_filing.py (preserved
unchanged) to accept an explicit --form argument, needed because 36b
hardcodes form=="10-K". Used here to lock MSFT's three FY2024 10-Q
filings (Q1/Q2/Q3) for the quarterly proof — every other function is
copied UNCHANGED from 36b (same SEC endpoints, same header/timeout/
delay policy, same manifest format, same locked-filings directory
layout). No ticker-specific logic anywhere.
"""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download an exact SEC filing package using ticker, form, and report date."
    )
    ...
    parser.add_argument("--form", required=True, help="Exact SEC form type, e.g. 10-Q or 10-K.")
...
def sec_get_bytes(url: str) -> bytes:
    ...
    try:
        ...
        try:
            ...
            response_body = ""
        raise RuntimeError(
            f"שגיאת HTTP מה־SEC.\nקוד: {error.code}\nכתובת: {url}\nתגובה: {response_body[:500]}"
        ) from error
    except URLError as error:
        raise RuntimeError(f"לא ניתן להתחבר ל־SEC.\nכתובת: {url}\nשגיאה: {error}") from error
...
def sec_get_json(url: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"ה־SEC לא החזיר JSON תקין.\nכתובת: {url}") from error
...
def find_company_record(ticker: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"לא נמצאה חברה יחידה עבור Ticker {ticker}.\nמספר התאמות: {len(matches)}")
    return matches[0]


def recent_filings_to_frame(submissions: dict[str, Any]) -> pd.DataFrame:
...
def historical_filings_to_frames(submissions: dict[str, Any]) -> list[pd.DataFrame]:
    ...
    for item in historical_files:
        ...
        if not file_name:
            continue
        historical_url = f"{SEC_DATA_URL}/submissions/{file_name}"
        historical_json = sec_get_json(historical_url)
...
def load_all_filings(cik: int) -> pd.DataFrame:
    ...
    submissions_url = f"{SEC_DATA_URL}/submissions/CIK{cik_padded}.json"
    submissions = sec_get_json(submissions_url)
    frames = []
    recent_frame = recent_filings_to_frame(submissions)
    if not recent_frame.empty:
        frames.append(recent_frame)
    frames.extend(historical_filings_to_frames(submissions))
    if not frames:
        raise RuntimeError("לא נמצאה היסטוריית הגשות.")
...
def select_exact_filing(filings: pd.DataFrame, form: str, report_date: str) -> pd.Series:
    ...
    if missing_columns:
        raise RuntimeError("חסרים שדות בהיסטוריית ההגשות:\n" + "\n".join(sorted(missing_columns)))

    candidates = filings[(filings["form"] == form) & (filings["reportDate"] == report_date)].copy()

    if len(candidates) != 1:
        print()
        print(f"עשר הגשות {form} האחרונות:")
        recent = filings[filings["form"] == form][
            ["reportDate", "filingDate", "accessionNumber", "primaryDocument"]
        ].sort_values("reportDate", ascending=False).head(10)
        print(recent.to_string(index=False))
        raise RuntimeError(
            f"לא נמצאה הגשת {form} יחידה ל־reportDate המבוקש.\n"
            f"Report date: {report_date}\nמספר התאמות: {len(candidates)}"
        )
...
def get_index_items(filing_index: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(items, list):
        raise RuntimeError("מבנה index.json אינו תקין.")
    return items
...
def download_filing_files(
    filing_base_url: str, output_directory: Path, index_items: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    output_directory.mkdir(parents=True, exist_ok=True)
    records = []
    for item in index_items:
        file_name = str(item.get("name", "")).strip()
        if not file_name or not should_download(file_name):
            continue
        ...
        records.append({
            ...
            "local_file": str(local_file.resolve()), "downloaded_size": len(content),
            "sec_reported_size": item.get("size", ""), "last_modified": item.get("last-modified", ""),
        })
        print(f"הורד: {file_name}")
    if not records:
        raise RuntimeError("לא הורדו קבצים מחבילת ההגשה.")
    return records


def save_manifests(
    ticker: str, company_name: str, cik: int, filing: pd.Series, filing_base_url: str,
    output_directory: Path, downloaded_records: list[dict[str, Any]],
) -> tuple[Path, Path]:
    ...
    if not primary_document_path.exists():
        raise FileNotFoundError(f"ה־Primary Document לא נמצא לאחר ההורדה:\n{primary_document_path}")

    filing_manifest = {
        "ticker": ticker, "company_name": company_name, "cik": cik,
        "form": str(filing["form"]), "report_date": str(filing["reportDate"]),
        "filing_date": str(filing["filingDate"]), "accession_number": str(filing["accessionNumber"]),
        "primary_document": primary_document,
        "primary_document_path": str(primary_document_path.resolve()),
    ...
    json_manifest_file = output_directory / "locked_filing_manifest.json"
    json_manifest_file.write_text(
        json.dumps(filing_manifest, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    csv_manifest_file = output_directory / "downloaded_files_manifest.csv"
    pd.DataFrame(downloaded_records).to_csv(csv_manifest_file, index=False, encoding="utf-8-sig")
    return json_manifest_file, csv_manifest_file
...
def main() -> None:
    ...
    print()
    print("=" * 110)
    print("ACCESSION-LOCKED SEC DOWNLOAD (any form)")
    print("=" * 110)
    ...
    accession_compact = accession_number.replace("-", "")
    output_directory = DATA_DIR / "sec_filings_locked" / ticker / accession_compact

    filing_index = load_filing_index(filing_base_url)
    index_items = get_index_items(filing_index)

    print()
    print("מוריד את חבילת ההגשה...")
    downloaded_records = download_filing_files(
        filing_base_url=filing_base_url, output_directory=output_directory, index_items=index_items
    )

    json_manifest_file, csv_manifest_file = save_manifests(
        ticker=ticker, company_name=company_name, cik=cik, filing=filing,
    ...
    print(f"Report date: {report_date}")
    print(f"Filing date: {filing_date}")
    print(f"מספר קבצים שהורדו: {len(downloaded_records)}")
    print()
    print(f"קובץ ראשי:\n{primary_document_path.resolve()}")
    print()
    print(f"Manifest:\n{json_manifest_file.resolve()}")
    print()
    print(f"רשימת קבצים:\n{csv_manifest_file.resolve()}")
    print()
    print(f"ההגשה נבחרה לפי Form={form} ו־reportDate מדויק.")
