...
#
# v12 (this file, 58, copied from 57/v11): bounded Debt Classification
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v12_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v12_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v12_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v12_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v12_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v12_orchestration.log"
        ),
    }
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v12 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
