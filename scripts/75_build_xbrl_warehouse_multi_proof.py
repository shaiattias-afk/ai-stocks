"""
Bounded WAREHOUSE GENERALIZATION proof — extends scripts/73 (which
proved the warehouse architecture on one filing, AMZN 2024-12-31,
approved as decision 1 in docs/DECISIONS_LOG.md D-022) to 3 more
already accession-locked 10-Ks (MSFT 2024-06-30, META 2024-12-31, NVDA
2024-01-28), each parsed with Arelle exactly ONCE, each closed before
the next is opened, all written into the SAME proof warehouse
(data/database/xbrl_warehouse_proof.duckdb) alongside AMZN's existing
rows — never overwriting them (idempotent per accession_number, see
create_schema/write_table below: CREATE TABLE IF NOT EXISTS + DELETE
scoped to the accession being (re)loaded, not a blanket table wipe —
this is the one deliberate change from scripts/73, required precisely
BECAUSE the warehouse is now proven to be reusable across filings, not
disposable per run).

Architecture distinction (binding, per docs/DECISIONS_LOG.md D-022):
...
Scope of this proof (per the user's explicit instruction): 3 already
locked filings, chosen to span different fiscal calendars (MSFT: June
30 FYE; META: December 31 FYE; NVDA: 52/53-week, ends last Sunday of
January), filing structures, extension-concept usage, and debt
presentations (MSFT: sum-of-siblings current_debt; NVDA: sum-of-
siblings with a single component; META: current_debt REVIEW_REQUIRED —
D-017 zero-inference condition 3 fails). Written to the SAME SEPARATE
proof database as scripts/73 — the production database
(data/database/ai_stock_agent.duckdb) is never opened by this script.
Not scaled beyond these 3 (+ AMZN already there = 4 of 45); no ticker-
or year-specific mapping rule anywhere below (every extraction function
here is generic — the exact same functions as scripts/73, unchanged —
it would run unchanged against any other 10-K).
...
SCRIPT_NAME = "75_build_xbrl_warehouse_multi_proof.py"
EXPECTED_FORM = "10-K"
...
def already_loaded(
    ...
    has a PASS warehouse_runs row for this script — Arelle is only
    reopened for filings not yet successfully loaded.
    """

    result = connection.execute(
        """
        SELECT count(*) FROM warehouse_runs
        WHERE accession_number = ? AND script_name = ? AND status = 'PASS'
        """,
        [accession_number, SCRIPT_NAME],
    ).fetchone()
