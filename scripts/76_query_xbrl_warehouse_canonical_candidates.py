...
def match_facts_from_warehouse(
    ...
    concept, USD unit, no dimensions, matching period end/instant date,
    correct period type, not nil, numeric value present — then decides
    PASS (exactly one distinct value) vs REVIEW_REQUIRED.
    """
...
def load_ground_truth(ticker: str, report_date: str) -> dict[str, object]:
    prefix = f"{ticker.lower()}_{report_date.replace('-', '')}"
    path = DATA_DIR / f"{prefix}_engine_v16_result.json"
...
def main() -> None:
    ...
    overall_report: list[dict[str, object]] = []
    ...
    for ticker, report_date, accession_number in TARGET_FILINGS:
        ...
        resolved_cache: dict[str, dict[str, object]] = {}
        # long_term_debt must be resolved before current_debt (zero-
        # inference condition 3 needs it) and before total_debt.
        ordered_metrics = ["long_term_debt"] + [
            m for m in TARGET_METRICS if m != "long_term_debt"
        ]

        reconstructed: dict[str, dict[str, object]] = {}

        for metric_name in ordered_metrics:
            result = reconstruct_metric(
                connection, accession_number, presentation, metric_name,
                report_date, resolved_cache,
            )
            reconstructed[metric_name] = result

            if metric_name not in ("current_debt",):
                resolved_cache[metric_name] = result

        query_seconds = time.perf_counter() - query_start

        ground_truth = load_ground_truth(ticker, report_date)

        print(f"DuckDB-only reconstruction time: {query_seconds:.6f}s")
        print(
            f"{'metric':22s} {'DuckDB status':16s} {'DuckDB value':>18s}  "
            f"{'live status':16s} {'live value':>18s}  match"
        )
        ...
        for metric_name in TARGET_METRICS:
            recon = reconstructed[metric_name]
            truth = ground_truth.get(metric_name, {})
            truth_value = (
                truth.get("value")
                if truth.get("value") is not None
                else truth.get("selected_value")
            )
            truth_status = truth.get("status")

            recon_value = recon.get("value")
            recon_status = recon.get("status")

            values_match = (
                recon_value == truth_value
                or (
                    recon_value is not None
                    and truth_value is not None
                    and abs(recon_value - truth_value) < 1
                )
                or (recon_value is None and truth_value is None)
            )
            status_match = recon_status == truth_status
            is_match = values_match and status_match
            ...
            print(
                f"{metric_name:22s} {recon_status or '':16s} "
                f"{str(recon_value):>18s}  {truth_status or '':16s} "
                f"{str(truth_value):>18s}  {'OK' if is_match else 'MISMATCH'}"
            )
    ...
    print("\n" + "=" * 100)
    print("SUMMARY")
    for entry in overall_report:
        print(
            f"  {entry['ticker']:6s} {entry['report_date']:12s} "
            f"{entry['matches']}/{entry['total']} matched, "
            f"query_seconds={entry['query_seconds']:.6f}, "
