"""
Quarterly proof (AMZN FY2024) — warehouse loader for the 3 already-
locked 10-Q filings: AMZN Q1 FY2024 (2024-03-31), Q2 FY2024
(2024-06-30), Q3 FY2024 (2024-09-30). Copied from
scripts/108_build_xbrl_warehouse_msft_10q_fy2024.py with only the
following changes: ticker/target filings (MSFT -> AMZN), script name/
labels, and ONE added bounded control-flow change — each filing load
now runs as a separate Python child process via
`subprocess.run(..., timeout=300)` instead of an in-process call, so a
single hung/slow Arelle load cannot block the others. Every extraction
function, the warehouse schema, and the Arelle-loading logic itself
(`load_one_filing`) are copied UNCHANGED from scripts/108 — no
redesign, no generic framework.

These filings were already locked (form=10-Q, exact reportDate,
accessionNumber, filingDate, primaryDocument — via
scripts/107_download_accession_locked_filing_any_form.py) and saved to
the existing sec_filings_locked location. Parsed with Arelle exactly
once each, written into the SAME warehouse
(data/database/xbrl_warehouse_proof.duckdb), idempotent per
accession_number.

AMZN's FY2024 10-K (2024-12-31) was already warehoused previously and
is NOT touched by this script. No other company is touched. No
quarterly metric extraction happens here.

Usage:
  Parent mode (default): python scripts/110_...py
    Launches this SAME script once per target filing, each as its own
    child process, each with a real 300-second timeout.
  Worker mode: python scripts/110_...py --single-report-date YYYY-MM-DD
    Loads exactly ONE filing synchronously, using the existing verified
    Arelle logic (load_one_filing), unchanged from scripts/108.
"""
...
SCRIPT_NAME = "110_build_xbrl_warehouse_amzn_10q_fy2024.py"
EXPECTED_FORM = "10-Q"
...
def run_worker(report_date: str) -> None:
...
def run_parent() -> None:
    ...
    for ticker, report_date in TARGET_FILINGS:
        ...
        try:
            result = subprocess.run(
                [sys.executable, str(Path(__file__).resolve()), "--single-report-date", report_date],
    ...
    print("\n" + "=" * 100)
    print("AMZN FY2024 10-Q WAREHOUSE LOAD — SUMMARY")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--single-report-date", default=None,
                         help="Worker mode: load exactly this one AMZN report_date synchronously.")
...
if __name__ == "__main__":
    ...
    if arguments.single_report_date:
        run_worker(arguments.single_report_date)
    else:
        run_parent()
