"""Computes and loads Scoring Model V1 (docs/SCORING_MODEL_V1_BLUEPRINT.md
Stage 3) for the 45 frozen company-years: the 9 raw factors
(scoring_inputs_v1) and their fiscal-year percentile-rank composite
(scoring_composite_v1). Reads ONLY already-frozen, point-in-time-gated
production data -- no new extraction, no external data.

Creates both tables (CREATE TABLE IF NOT EXISTS, idempotent) on first
run, then appends through the write guard exactly like every other
production loader in this project (D-047).

    --check-only   compute everything, write nothing, report a summary
    --execute      back up, create tables if needed, append through the
                   guard, re-verify read-only
"""
...
ENGINE_VERSION = "v1-scoring-model (scripts/192)"
SCORING_VERSION = "ScoringModelV1"
RESULT_PATH = DATA_DIR / "scoring_model_v1_load_result.json"
...
CREATE TABLE IF NOT EXISTS {INPUTS_TABLE} (
    ...
    revenue_growth DOUBLE, revenue_growth_status VARCHAR,
    roic_level DOUBLE, roic_level_status VARCHAR,
    roic_trend DOUBLE, roic_trend_status VARCHAR,
    operating_margin DOUBLE, operating_margin_status VARCHAR,
    fcf_margin DOUBLE, fcf_margin_status VARCHAR,
    fcf_growth DOUBLE, fcf_growth_status VARCHAR,
    balance_sheet_strength_ratio DOUBLE, balance_sheet_strength_ratio_status VARCHAR,
    capex_discipline_deviation DOUBLE, capex_discipline_deviation_status VARCHAR,
    capex_discipline_trailing_years_used INTEGER,
    distance_from_high DOUBLE, distance_from_high_status VARCHAR,
    distance_from_high_price_date DATE, distance_from_high_trailing_high DOUBLE,
    distance_from_high_price DOUBLE,
...
CREATE TABLE IF NOT EXISTS {COMPOSITE_TABLE} (
    ...
    weight_covered DOUBLE,
    factors_used INTEGER,
...
def main() -> None:
    ...
    target_company_years = connection.execute(
        """
        ...
        WHERE er.engine_version NOT LIKE '%scripts/188%' AND er.engine_version NOT LIKE '%scripts/191%'
        ORDER BY 1, 2
        """
    ).fetchall()
    print(f"target company-years (frozen 45): {len(target_company_years)}")

    loaded_at = datetime.now(timezone.utc)
    all_inputs = []
    for ticker, report_date, fiscal_year in target_company_years:
    ...
    backup_path = BACKUP_DIR / f"ai_stock_agent_pre_scoring_v1_{stamp}.duckdb"
    ...
    ddl_connection.execute(INPUTS_DDL)
    ddl_connection.execute(COMPOSITE_DDL)
    ddl_connection.close()
    print(f"tables ensured: {INPUTS_TABLE}, {COMPOSITE_TABLE}")
    ...
    frozen_900 = verify.execute("SELECT COUNT(*) FROM financial_metric_results").fetchone()[0]
    verify.close()

    ok = (inputs_count == len(inputs_rows) and composite_count == len(composite_rows)
          and distinct_tickers == 9 and dup_keys == 0)
    payload.update({
        "inputs_rows_written": inputs_count, "composite_rows_written": composite_count,
        "distinct_tickers": distinct_tickers, "duplicate_keys": dup_keys,
        "financial_metric_results_unchanged_at_30180_or_current": frozen_900,
