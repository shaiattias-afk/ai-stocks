"""
Annual XBRL Warehouse completion gate — Step 2: loads the 12 filings
the audit (scripts/114) found LOCKED_NOT_WAREHOUSED. All 12 are
already locked on disk (form=10-K, exact reportDate/accessionNumber/
filingDate/primaryDocument, already confirmed by the audit) — this
script does NOT download anything. It ONLY parses each already-locked
filing with Arelle exactly once, using the SAME extraction logic and
warehouse schema copied unchanged from scripts/108 (via scripts/110/112),
and the SAME parent/worker subprocess pattern proven in
scripts/110_build_xbrl_warehouse_amzn_10q_fy2024.py — each filing loads
in its own child process with a real 300-second timeout; on timeout or
failure the filing is recorded and the loop continues, no retry.

Generalized ONLY in the one place required: TARGET_FILINGS now spans 3
tickers (MSFT, NVDA, ORCL) instead of 1, and worker-mode CLI arguments
take both --single-ticker and --single-report-date (the single-company
scripts only needed the date, since TICKER was a fixed constant). No
other change to the loader, schema, or Arelle logic.

Missing filings (12), all already locked:
  MSFT 10-K 2020-06-30  accession 0001564590-20-034944
  MSFT 10-K 2021-06-30  accession 0001564590-21-039151
  MSFT 10-K 2022-06-30  accession 0001564590-22-026876
  MSFT 10-K 2023-06-30  accession 0000950170-23-035122
  NVDA 10-K 2021-01-31  accession 0001045810-21-000010
  NVDA 10-K 2022-01-30  accession 0001045810-22-000036
  NVDA 10-K 2023-01-29  accession 0001045810-23-000017
  ORCL 10-K 2020-05-31  accession 0001564590-20-030125
  ORCL 10-K 2021-05-31  accession 0001564590-21-033616
  ORCL 10-K 2022-05-31  accession 0001564590-22-023675
  ORCL 10-K 2023-05-31  accession 0000950170-23-028914
  ORCL 10-K 2024-05-31  accession 0000950170-24-075605

Does NOT compute or write any canonical financial metric. Does NOT
touch the production database. Does NOT touch Annual V1.
"""
...
SCRIPT_NAME = "115_annual_warehouse_load_missing.py"
EXPECTED_FORM = "10-K"
...
def run_worker(ticker: str, report_date: str) -> None:
    """Worker mode: loads exactly ONE filing synchronously, using the
    existing verified Arelle logic (load_one_filing) unchanged. Invoked
    by the parent as a child process so a real timeout can be enforced
    from outside."""
...
def run_parent() -> None:
    ...
    print(f"קבצים למעבר ({len(TARGET_FILINGS)}): {TARGET_FILINGS}")
    print(f"child_process_timeout_seconds = {CHILD_PROCESS_TIMEOUT_SECONDS}")
    total_start = time.perf_counter()
    summaries: list[dict[str, object]] = []

    for ticker, report_date in TARGET_FILINGS:
        filing_start = time.perf_counter()
        print(f"\n=== launching child process for {ticker} {report_date} "
              f"(timeout={CHILD_PROCESS_TIMEOUT_SECONDS}s) ===")

        try:
            result = subprocess.run(
                [sys.executable, str(Path(__file__).resolve()),
                 "--single-ticker", ticker, "--single-report-date", report_date],
            ...
            if result.returncode != 0:
                summary = {"ticker": ticker, "report_date": report_date, "status": "FAIL",
                           "filing_elapsed_seconds": round(filing_elapsed, 3),
                           "error": f"child process exited with code {result.returncode}"}
            else:
                worker_line = next(
                    (line for line in result.stdout.splitlines() if line.startswith("WORKER_RESULT_JSON=")),
                    None,
                )
                if worker_line is None:
                    summary = {"ticker": ticker, "report_date": report_date, "status": "FAIL",
                               "filing_elapsed_seconds": round(filing_elapsed, 3),
                               "error": "child process exited 0 but no WORKER_RESULT_JSON line found"}
                else:
            ...
            filing_elapsed = time.perf_counter() - filing_start
            print(f"TIMEOUT after {CHILD_PROCESS_TIMEOUT_SECONDS}s — terminating child process, "
                  f"marking TIMEOUT, continuing with next filing (no retry).")
            if exc.stdout:
                print("--- partial child stdout before timeout ---")
                print(exc.stdout if isinstance(exc.stdout, str) else exc.stdout.decode("utf-8", errors="ignore"))
            summary = {"ticker": ticker, "report_date": report_date, "status": "TIMEOUT",
                       "filing_elapsed_seconds": round(filing_elapsed, 3),
                       "error": f"child process exceeded {CHILD_PROCESS_TIMEOUT_SECONDS}s and was terminated"}

        summaries.append(summary)
        print(f"[{ticker} {report_date}] status={summary.get('status')} "
              f"elapsed={summary.get('filing_elapsed_seconds')}s")

    print("\n" + "=" * 100)
    print("ANNUAL WAREHOUSE — MISSING FILINGS LOAD — SUMMARY")
    for summary in summaries:
        print(f"  {summary['ticker']:6s} {summary['report_date']:12s} "
              f"status={summary.get('status')} elapsed={summary.get('filing_elapsed_seconds')}s")
    pass_count = sum(1 for s in summaries if s.get("status") in ("PASS", "SKIPPED_ALREADY_LOADED"))
    print(f"PASS/SKIPPED: {pass_count} / {len(summaries)}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--single-ticker", default=None,
                         help="Worker mode: the ticker to load (used together with --single-report-date).")
    parser.add_argument("--single-report-date", default=None,
                         help="Worker mode: load exactly this one report_date synchronously.")
