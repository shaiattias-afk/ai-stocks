...
# prior proofs (scripts 37c through 41).
...
BUILT_IN_METRICS: dict[str, MetricDefinition] = {
    ...
    "operating_income": MetricDefinition(
        ...
        role_include_pattern=r"operations|income",
        role_exclude_pattern=r"comprehensive",
        mention_pattern=r"operating\s+(?:income|loss)",
        exclude_label_pattern=r"per\s+share|weighted\s+average|margin|percentage",
        attributable_pattern=None,
        plain_pattern=r"^\s*operating\s+(?:income\s*\(loss\)|income|loss)\s*$",
    ),
}
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": DATA_DIR / f"{prefix}_engine_presentation.csv",
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_result.json",
        "arelle_log_file": DATA_DIR / f"{prefix}_engine_arelle_child.log",
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_orchestration.log"
        ),
    }
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
