...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v6b_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v6b_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v6b_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v6b_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v6b_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v6b_orchestration.log"
        ),
    }
...
def resolve_current_debt_components(
    ...
    stopping at the first tier that succeeds. Raises TargetRowNotFound
    (→ REVIEW_REQUIRED) if no tier resolves unambiguously.
    """
    ...
    if sibling_components is None:
        raise TargetRowNotFound(
            "לא נמצא אף רכיב חוב שוטף מותר (Short-Term Debt / "
            "Commercial Paper / Current Portion of Long-Term Debt וכו') "
            "בתוך Statement role ראשי של Balance Sheet. לא הוסק "
            "current_debt = 0."
        )
...
def engine_child_worker(
    ...
    try:
        ...
        with Session() as session:
            ...
            # several sibling components to be summed (policy D-016).
            ...
            for metric_name in row_identify_metric_names:
                try:
                    if metric_name == "current_debt":
                        rows = resolve_current_debt_components(
                            model_xbrl, presentation
                        )
                    else:
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v6 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
