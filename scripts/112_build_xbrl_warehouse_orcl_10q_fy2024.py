"""
Quarterly proof (ORCL FY2024) — warehouse loader for the 3 already-
locked 10-Q filings: ORCL Q1 FY2024 (2023-08-31), Q2 FY2024
(2023-11-30), Q3 FY2024 (2024-02-29). Copied from
scripts/110_build_xbrl_warehouse_amzn_10q_fy2024.py with only the
following changes: ticker/target filings (AMZN -> ORCL), script name/
labels. The bounded control-flow change from scripts/110 (each filing
load runs as a separate Python child process via
`subprocess.run(..., timeout=300)`) is preserved unchanged. Every
extraction function, the warehouse schema, and the Arelle-loading
logic itself (`load_one_filing`) are copied UNCHANGED from scripts/108
via scripts/110 — no redesign, no generic framework.
...
SCRIPT_NAME = "112_build_xbrl_warehouse_orcl_10q_fy2024.py"
EXPECTED_FORM = "10-Q"
...
def run_parent() -> None:
    ...
    print("\n" + "=" * 100)
    print("ORCL FY2024 10-Q WAREHOUSE LOAD — SUMMARY")
