"""
ORCL FY2024 quarterly extraction proof — bounded, single-company proof
of concept for deriving quarterly (Q1-Q4) values from 10-Q/10-K
filings. Copied from the verified scripts/111_amzn_fy2024_quarterly_proof.py
with ONLY the following changed: company label, the FILINGS dict
(ORCL's own accessions/report dates/filing dates), and the output file
paths/labels. The quarter-duration classification, fact-selection,
derivation logic, and the precision-duplicate reconciliation
(`facts_for_concept` calling s89's `_reconcile_same_context_precision_
duplicates_from_warehouse`) are untouched.

Filings used (all already locked + warehoused, point-in-time correct):
  Q1 10-Q  report_date=2023-08-31  filed 2023-09-12
  Q2 10-Q  report_date=2023-11-30  filed 2023-12-12
  Q3 10-Q  report_date=2024-02-29  filed 2024-03-12
  FY 10-K  report_date=2024-05-31  filed 2024-06-20

Metrics: revenue, operating_income, pretax_income, income_tax_expense
(tax_expense), operating_cash_flow, capex.

Quarter logic (exactly as specified, unchanged from scripts/109/111):
  ...
  6. Q4's point-in-time availability date = the FY2024 10-K's own
     filing_date (2024-06-20) — never an earlier date.
...
data/quarterly_proof_orcl_fy2024.json and .csv.
"""
...
def main() -> None:
    ...
    print("=" * 100)
    print("ORCL FY2024 QUARTERLY EXTRACTION PROOF")
    print("=" * 100)
