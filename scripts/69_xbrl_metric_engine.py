...
#
# v15 (this file, 69, copied from 60/v14): accounting policy D-020 —
# structural fallback for `pretax_income`. Root-cause analysis of the
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v15_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v15_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v15_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v15_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v15_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v15_orchestration.log"
        ),
    }
