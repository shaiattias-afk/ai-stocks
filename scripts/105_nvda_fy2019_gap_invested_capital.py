"""
D-034 — computes current_debt/long_term_debt/total_debt/cash/short_term_
investments/stockholders_equity/adjusted_net_debt/invested_capital for
NVDA's newly-warehoused FY2019 (2019-01-27) filing, applying the EXACT
SAME already-approved policy engine as scripts/92 (Policies A/B/C) —
reused via importlib, not duplicated. Same pattern as scripts/98/101
(D-029/D-031), which did the identical thing for AMZN/GOOGL FY2020,
META FY2019, and CRWD FY2021.

Treats NVDA FY2019 as a genuine addition to the historical dataset (its
...
Point-in-time correctness: only facts from NVDA FY2019's own warehouse
entry are read — no fact from NVDA FY2020 is read here, and vice versa.
...
ENGINE_VERSION = "v1-nvda-fy2019-gap-invested-capital (scripts/105, D-034)"
...
def write_result_to_production_db(prod_connection: duckdb.DuckDBPyConnection, result: dict) -> int:
    ...
    for metric_name in WRITTEN_METRICS:
        ...
        if status == "REVIEW_REQUIRED":
            ...
            validation_reason = (
                f"D-034 (NVDA first-fiscal-year addition, reusing scripts/92's "
...
def main() -> None:
    ...
    for ticker, report_date, accession_number in TARGET_FILINGS:
        ...
        print("  arelle_required=NO (already warehoused by scripts/104)")
    ...
    print("=" * 100)
    print(f"NVDA FY2019 GAP INVESTED CAPITAL COMPLETE — total_elapsed_seconds = {total_elapsed:.3f}")
    print("=" * 100)
