...
#
# v4 (this file, 42/43/44/45 unmodified): adds four Balance Sheet metrics
...
# dependency order.
#
# v4.1 (this file, 46; 45 left unmodified): fixes an instant-date-off-
# by-one-day bug in match_facts — see the comment at the instant-context
# branch below.
# =============================================================================
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v4b_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v4b_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v4b_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v4b_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v4b_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v4b_orchestration.log"
        ),
    }
    ...
    for fact_index, fact in enumerate(model_xbrl.facts):
        ...
        if is_duration:
            ...
            if instant_dt is not None:
                # Empirically verified against real Arelle output (not
                # assumed): instant dates use the exact same exclusive
                # "midnight of the following day" convention as duration
                # end dates. A balance sheet dated 2024-06-30 is returned
                # by Arelle as instantDatetime 2024-07-01T00:00:00, so
                # the same -1 day adjustment is required here too. An
                # earlier version of this engine (45) assumed instant
                # dates needed no adjustment, which silently rejected
                # every real balance-sheet fact (off by one day) and
                # produced false REVIEW_REQUIRED results.
                period_end = (
