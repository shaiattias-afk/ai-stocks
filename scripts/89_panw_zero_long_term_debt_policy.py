...
TARGET_FILINGS: list[tuple[str, str, str]] = [
    ("PANW", "2023-07-31", "0001327567-23-000024"),
    ("PANW", "2024-07-31", "0001327567-24-000029"),
]
# PANW 2025-07-31 is NOT in the warehouse and is explicitly excluded —
# loading it would require Arelle, which this task (D-026) forbids.
...
def reconstruct_metric(
    ...
    if metric_name == "long_term_debt":
        ...
        try:
            ...
            # Re-derive current_debt's own claimed components directly
            # (not from resolved_cache — compute_company_year resolves
            # long_term_debt BEFORE current_debt, so the cache would be
            # empty here; calling the same resolver current_debt itself
            # uses guarantees the correct claimed set regardless of call
            # order, and is read-only/side-effect-free).
            ...
            if len(ancestry_matches) == 1:
                ...
                # Approved maturity/zero-debt policy, item 6: if the
                # filing proves no financial debt remains outstanding
                # beyond what current_debt already claimed, long_term_
                # debt is zero — proven, not assumed, by confirming
                # find_debt_vocabulary_rows (the SAME broadened D-019
                # debt-vocabulary search, across every balance-sheet
                # role, excluding only the concepts current_debt already
                # claimed) finds ZERO further candidates anywhere. If
                # even one unclaimed debt-vocabulary row exists but
                # can't be classified current/noncurrent, this does NOT
                # prove zero — it remains a genuine ambiguity,
                ...
                for role_uri in balance_sheet_role_uris:
                    found = find_debt_vocabulary_rows(
                        presentation, role_uri, already_claimed
                    )
                    unclaimed_candidates.extend(found["concept_qname"].tolist())
                ...
                return {
                    ...
                    "lineage": {
                        ...
                        "balance_sheet_roles_searched": balance_sheet_role_uris,
                    },
                }
            else:
                return {
...
def main() -> None:
    # D-026: direct policy application, no investigation/control-gate
    # phase (per explicit instruction) — PANW 2023-07-31 and 2024-07-31
    # only; 2025-07-31 is NOT in the warehouse and is explicitly skipped
    # rather than loaded via Arelle (forbidden by this task).
    total_start = time.perf_counter()
    connection = duckdb.connect(database=str(WAREHOUSE_DB_PATH), read_only=True)
    all_results: dict[str, dict[str, object]] = {}

    print("PANW 2025-07-31 SKIPPED: not present in the XBRL warehouse; "
          "loading it would require Arelle, which this task forbids.")
    ...
    for ticker, report_date, accession_number in TARGET_FILINGS:
        ...
        print(f"  long_term_debt: status={result['long_term_debt']['status']} "
              f"value={result['long_term_debt'].get('value')} "
              f"tier={result['long_term_debt'].get('selection_tier')} "
              f"basis={result['long_term_debt'].get('basis')}")
        lineage = result['long_term_debt'].get('lineage')
        if lineage:
    ...
    for key, result in all_results.items():
        ...
        # confirm current_debt (unaffected by this fix) stayed identical
        old_cd = ground_truth.get("current_debt", {})
        old_cd_value = old_cd.get("value") if old_cd.get("value") is not None else old_cd.get("selected_value")
        new_cd = result["current_debt"]
        match = old_cd.get("status") == new_cd.get("status") and (
            old_cd_value == new_cd.get("value")
            or (old_cd_value is not None and new_cd.get("value") is not None
                and abs(old_cd_value - new_cd.get("value")) < 1e-6)
        )
        unaffected_confirmed.append(f"{ticker} {report_date} current_debt: match={match}")
    ...
    print(f"\ncurrent_debt unaffected-check:")
    for line in unaffected_confirmed:
        print(f"  {line}")

    print(f"\ntotal_elapsed_seconds = {total_elapsed:.4f}")

    output_path = DATA_DIR / "d026_panw_zero_ltd_results.json"
