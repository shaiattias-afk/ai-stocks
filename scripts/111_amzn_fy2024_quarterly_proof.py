"""
AMZN FY2024 quarterly extraction proof — bounded, single-company proof
of concept for deriving quarterly (Q1-Q4) values from 10-Q/10-K
filings. Copied from the verified scripts/109_msft_fy2024_quarterly_proof.py
with ONLY the following changed: company label, the FILINGS dict
(AMZN's own accessions/report dates/filing dates), and the output file
paths/labels. The quarter-duration classification, fact-selection, and
derivation logic are untouched.

Filings used (all already locked + warehoused, point-in-time correct):
  Q1 10-Q  report_date=2024-03-31  filed 2024-05-01
  Q2 10-Q  report_date=2024-06-30  filed 2024-08-02
  Q3 10-Q  report_date=2024-09-30  filed 2024-11-01
  FY 10-K  report_date=2024-12-31  filed 2025-02-07

Metrics: revenue, operating_income, pretax_income, income_tax_expense
(tax_expense), operating_cash_flow, capex.

Quarter logic (exactly as specified, unchanged from scripts/109):
  ...
  4. Fallback: Q2 quarter = Q2 YTD (6mo) - Q1 YTD (3mo, i.e. Q1's own
     value); Q3 quarter = Q3 YTD (9mo) - Q2 YTD (6mo).
  ...
  6. Q4's point-in-time availability date = the FY2024 10-K's own
     filing_date (2025-02-07) — never an earlier date.
...
data/quarterly_proof_amzn_fy2024.json and .csv.
"""
...
JSON_OUTPUT_PATH = DATA_DIR / "quarterly_proof_amzn_fy2024.json"
CSV_OUTPUT_PATH = DATA_DIR / "quarterly_proof_amzn_fy2024.csv"
...
FILINGS = {
    "Q1": {"form": "10-Q", "report_date": "2024-03-31", "filing_date": "2024-05-01",
           "accession_number": "0001018724-24-000083"},
    "Q2": {"form": "10-Q", "report_date": "2024-06-30", "filing_date": "2024-08-02",
           "accession_number": "0001018724-24-000130"},
    "Q3": {"form": "10-Q", "report_date": "2024-09-30", "filing_date": "2024-11-01",
           "accession_number": "0001018724-24-000161"},
    "FY": {"form": "10-K", "report_date": "2024-12-31", "filing_date": "2025-02-07",
...
def main() -> None:
    ...
    print("=" * 100)
    print("AMZN FY2024 QUARTERLY EXTRACTION PROOF")
    print("=" * 100)
    ...
    for metric_name in METRICS:
        ...
        # across all 4 AMZN filings, but resolved independently each
        ...
        for label, info in FILINGS.items():
            ...
            for quarter_label in ("Q1", "Q2", "Q3", "Q4"):
                ...
                csv_rows.append({
                    "ticker": "AMZN", "fiscal_year": "FY2024", "fiscal_quarter": quarter_label,
    ...
    # --- write outputs ---
    output = {
        "company": "AMZN", "fiscal_year": "FY2024", "fiscal_year_end": "2024-12-31",
