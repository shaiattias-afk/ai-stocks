...
#
# v10 (this file, 56, copied from 55/v9): generalization test on a 4th
...
BUILT_IN_METRICS: dict[str, MetricDefinition] = {
    ...
    "capex": MetricDefinition(
        ...
        mention_pattern=(
            ...
            r"(?:additions|purchases)\s+(?:related\s+)?(?:to|of)\s+property"
        ),
        ...
        attributable_pattern=None,
        # The trailing "(?:\s+and\s+intangible\s+assets)?" and the
        # "(?:related\s+)?" infix (added while testing NVIDIA) are pure
        # broadenings — a fourth real SEC-filer phrasing for the same
        # us-gaap:PaymentsToAcquireProductiveAssets concept ("Purchases
        # related to property and equipment and intangible assets"),
        # alongside Oracle/Microsoft/Meta's existing three. Every
        # previously-matching label still matches unchanged.
        plain_pattern=(
            r"^\s*capital\s+expenditures?\s*$|"
            r"^\s*(?:additions|purchases)\s+(?:related\s+)?(?:to|of)\s+property"
            r"(?:,?\s+plant)?\s+and\s+equipment"
    ...
    "pretax_income": MetricDefinition(
        ...
        # "...income taxes" already handled — a genuine, common SEC-filer
        # wording difference for the same concept, not specific to any
        # one company; every previously-matching label still matches.
        plain_pattern=(
            r"^\s*income\s*(?:\(loss\))?\s*before\s+"
            r"(?:provision\s+for\s+)?income\s+tax(?:es)?\s*$"
        ),
    ),
    "income_tax_expense": MetricDefinition(
        ...
        # handled; every previously-matching label still matches.
        plain_pattern=(
            r"^\s*(?:provision|benefit)\s+for\s+income\s+tax(?:es)?\s*$"
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v10_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v10_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v10_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v10_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v10_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v10_orchestration.log"
        ),
    }
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v10 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
