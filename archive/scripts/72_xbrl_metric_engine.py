...
#
# v16 (this file, 72, copied from 69/v15): attempted accounting policy
# D-021 — extract current_debt directly from a debt-maturity schedule's
# earliest ("next twelve months") bucket, instead of only using that
# bucket to PROVE current_debt is not zero (the existing D-017 zero-
# inference check). Targeted at the "zero_inference_earliest_bucket_
# nonzero" REVIEW_REQUIRED root cause (60 potentially-resolved results,
# AMZN and GOOGL, all 5 years each — see docs/LAST_CLAUDE_REPORT.md).
#
# New function attempt_current_debt_from_maturity_bucket() locates the
# same earliest-bucket row D-017's condition 2 already finds, and
# evaluates it as a CANDIDATE current_debt VALUE (not just a zero-proof)
# subject to a strict evidence bar (role/period/unit already validated
# structurally like every other metric; presentation ancestor chain
# recorded via build_ancestor_chain; excludes any row whose concept or
# label indicates a lease, purchase obligation, interest payment, or
# other non-debt commitment). Wired in as: try the existing D-017 zero-
# inference proof first, completely unchanged; only if it fails does
# this new tier attempt to resolve a usable VALUE from the same bucket,
# and only if that also fails does current_debt fall through to
# REVIEW_REQUIRED exactly as before — never overriding an existing
# result, never guessing.
#
# PROOF-TEST RESULT (AMZN 2021-12-31, GOOGL 2021-12-31, both run before
# any broader rollout — see docs/LAST_CLAUDE_REPORT.md for the full
# report): direct inspection of both filings' actual "Future Principal
# Payments for Debt/Borrowings" disclosure roles shows the earliest
# bucket in BOTH is tagged with concept
# `us-gaap:LongTermDebtMaturitiesRepaymentsOfPrincipalInNextTwelveMonths`
# — its own local name states, unambiguously, that this is an
# undiscounted PRINCIPAL repayment amount, not a GAAP CARRYING amount
# (accounting requirement 1 explicitly prohibits using face/principal
# value). No separate carrying-amount fact exists for the current-year
# slice in isolation anywhere in either filing's presentation data — the
# schedule's only carrying-value fact is the aggregate "Total"
# row (concept `us-gaap:LongTermDebt`), which cannot be safely allocated
# between current/non-current portions from what is disclosed. This is
# not an AMZN/GOOGL-specific finding — a debt maturity/repayment
# schedule is, by the underlying US GAAP disclosure convention (ASC
# 835/470), inherently an undiscounted PRINCIPAL cash-flow schedule, not
# a carrying-value schedule, in virtually every 10-K. Per requirement 1
# and requirement 5 ("if the bucket is ambiguous or overlapping, keep
# REVIEW_REQUIRED"), CARRYING_VALUE_REJECT_CONCEPT_PATTERN below
# correctly rejects this concept and current_debt correctly stays
# REVIEW_REQUIRED for both proof-test filings — this is the check
# working as designed, not a bug. Per this task's own stop condition
# ("continue only if both proof tests pass safely"), the 10-company
# rerun, DuckDB update, and re-ranking were NOT performed — see the
# final report for the full recommendation.
#
# REGRESSION FOUND AND FIXED DURING THE REQUIRED 9-ANCHOR REGRESSION
# TEST (before any report was written): the first wiring attempted the
# new tier whenever attempt_current_debt_zero_inference raised ANY
# exception, not specifically the "earliest bucket nonzero" case this
# policy targets. Meta's zero-inference genuinely fails at D-017
# condition 3 (its maturity-schedule Total, $29.0B, does not reconcile
# with balance-sheet long_term_debt, $28.826B — a real, still-unresolved
# ~$174M discrepancy) while Meta's OWN earliest bucket is already zero
# (condition 2 already succeeds). The first wiring re-derived that same
# zero-valued bucket independently, found its concept name didn't match
# the principal/face-value rejection pattern, and wrongly accepted it as
# a resolved current_debt = 0 — silently masking the real, unrelated
# condition-3 problem, and cascading a false PASS into total_debt,
# adjusted_net_debt, and invested_capital. Fixed by adding an explicit
# scope guard: this tier now rejects outright whenever the fetched
# bucket value is exactly 0, since a zero bucket means D-017's own
# condition 2 already succeeds — any zero-inference failure in that
# situation is necessarily a DIFFERENT, still-unresolved condition (3
# or 4), which this policy is not scoped to fix and must never paper
# over. Re-verified clean on all 9 anchors after the fix (see final
# report); AMZN/GOOGL's own proof-test results were unaffected by this
# fix (their bucket values are genuinely nonzero).
#
# v15 (69, unmodified, still the engine of record for every metric
# except this proof test): accounting policy D-020 — structural
# fallback for `pretax_income`. Root-cause analysis of the
# 222 historical REVIEW_REQUIRED results (scripts/68) found 9 company-
# years (GOOGL FY2021-2023, CRWD FY2022-2023, PANW FY2021-2022, MU
# FY2021-2022) where pretax_income's label-based search failed — three
# distinct label variants (Google: bare "Total"; CrowdStrike/Palo Alto
# Networks: "Loss before (provision for) income taxes"; Micron: a
# middle clause before the existing "and ..." trailing-clause allowance)
# behind one general, verified structural fact: on every standard
# multi-step income statement, the pretax-income row is, by
# construction, the presentation-tree row immediately preceding
# income_tax_expense — same parent, same role, adjacent order — proven
# by direct inspection of Google's, CrowdStrike's, and Micron's actual
# presentation data before implementation (not assumed). New function
# find_pretax_income_by_structural_position() implements this as a
# fallback tier, used ONLY when the existing label-based search already
# found nothing — never overriding a successful label match, and never
# guessing when income_tax_expense itself isn't resolved or no
# qualifying immediate predecessor exists (fails closed to
# REVIEW_REQUIRED exactly as before). No ticker/year-specific logic, no
# concept-name tag list (the fallback never checks concept name, only
# structural adjacency + duration context + credit balance), no
# weakened validation, no assumed values. Touches only pretax_income's
# row identification — no other metric's logic changed.
#
# v14 (59->60, unmodified): second pass on Oracle's
# historical filings — FY2021's pretax_income label is "Income before
# benefit from (provision for) income taxes" (a "benefit from" variant
# of the "provision for" wording pretax_income's plain_pattern already
# handled — analogous to the same "for"/"from" drift already fixed for
# income_tax_expense in v13). Added "benefit\s+from" as a second option
# alongside "provision\s+for". Not ticker-specific; every previously-
# matching label in any prior company/year still matches unchanged.
#
# v13 (58->59, unmodified): first HISTORICAL multi-year
...
BUILT_IN_METRICS: dict[str, MetricDefinition] = {
    ...
    "pretax_income": MetricDefinition(
        ...
        # still start with the required core phrase. The
        # "(?:provision\s+for|benefit\s+from)\s+" alternative (added
        # while testing Oracle's FY2021 historical filing) allows
        # "Income before benefit from (provision for) income taxes" —
        # once the parenthetical aside is stripped, "benefit from" sits
        # where "provision for" already did. None of these changes are
        # ticker-specific; every previously-matching label still
        # matches.
        plain_pattern=(
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v16_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v16_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v16_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v16_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v16_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v16_orchestration.log"
        ),
    }
...
def find_pretax_income_by_structural_position(
    presentation: pd.DataFrame,
    income_tax_expense_row: dict[str, str] | None,
) -> dict[str, str] | None:
    """
    Structural fallback for `pretax_income`, used ONLY when the ordinary
    label-based search (identify_canonical_row) already found nothing
    unambiguous. General accounting pattern, verified against real
    presentation data across multiple filers before being implemented
    (Google, CrowdStrike, Micron — see docs/DECISIONS_LOG.md D-020):
    on every standard multi-step income statement, the row for pretax
    income is, by construction, the presentation-tree row immediately
    preceding the income-tax-expense row — same immediate parent, same
    role, directly adjacent presentation order — regardless of what
    label text the filer used for it (a filer may even leave it
    completely generic, e.g. a bare "Total"). This is a structural
    property of how every GAAP income statement is built (revenue -
    expenses -> operating income -> other income/expense -> PRETAX
    INCOME -> tax expense -> net income), not a label convention, and
    not specific to any filer, ticker, or fiscal year.

    Requires `income_tax_expense_row` to already be resolved (by the
    ordinary, already-verified income_tax_expense search) — if it is
    None, or no qualifying immediate predecessor exists, returns None
    (the caller must then fail closed to REVIEW_REQUIRED, never guess).
    """

    if income_tax_expense_row is None:
        return None

    role_uri = income_tax_expense_row["role_uri"]
    tax_concept = income_tax_expense_row["concept_qname"]

    role_rows = presentation[presentation["role_uri"] == role_uri]

    tax_matches = role_rows[role_rows["concept_qname"] == tax_concept]

    if len(tax_matches) != 1:
        return None

    tax_row = tax_matches.iloc[0]
    parent_qname = str(tax_row["parent_qname"])

    siblings = role_rows[
        (role_rows["parent_qname"] == parent_qname)
        & (~role_rows["is_abstract"].astype(bool))
    ].reset_index(drop=True)

    tax_positions = siblings.index[
        siblings["concept_qname"] == tax_concept
    ]

    if len(tax_positions) != 1 or tax_positions[0] == 0:
        return None

    candidate = siblings.iloc[tax_positions[0] - 1]

    # Structural sanity checks — never a label match, purely structural/
    # taxonomy-convention evidence: duration context (pretax income is a
    # period flow, like every other income-statement line), and a
    # credit balance (an income/subtotal line, not an expense line —
    # us-gaap:IncomeTaxExpenseBenefit itself is "debit"; the line
    # immediately before it representing income must be "credit").
    if str(candidate["period_type"]) != "duration":
        return None

    if str(candidate.get("balance", "")) != "credit":
        return None

    return {
        "role_uri": role_uri,
        "role_definition": str(candidate["role_definition"]),
        "concept_qname": str(candidate["concept_qname"]),
        "label": str(candidate["label"]),
        "period_type": str(candidate["period_type"]),
        "selection_tier": "structural_predecessor_of_income_tax_expense",
    }
...
# =============================================================================
# 3c-bis + 4c-bis. CURRENT DEBT FROM MATURITY-SCHEDULE BUCKET — attempted
# accounting policy D-021 (bounded, evidence-gated). Only ever attempted
# AFTER D-017's zero-inference proof above has already failed — never
# overrides a genuine zero-inference PASS, never runs when tiers 1-3
# already resolved current_debt directly. Targets the
# "zero_inference_earliest_bucket_nonzero" root cause: when the earliest
# maturity bucket is reliably nonzero, condition 2 above correctly
# refuses to prove current_debt = 0 — but the bucket's own reported
# amount may be usable AS current_debt's actual value, if and only if it
# is evidenced as a GAAP CARRYING amount (never an undiscounted
# face-value/principal repayment schedule amount — accounting
# requirement 1). See the v16 module docstring above for the concrete
# proof-test finding: on both filings actually tested (AMZN 2021-12-31,
# GOOGL 2021-12-31), the earliest bucket's own concept name
# (`...RepaymentsOfPrincipal...`) states outright that it is a principal
# amount, so this tier correctly REJECTS it and current_debt correctly
# stays REVIEW_REQUIRED — proven, not assumed, by inspecting the actual
# concept evidence below, never by a ticker-specific exception.
# =============================================================================


# A debt-maturity/repayment schedule bucket's own concept LOCAL NAME
# (not just its label, which can vary by filer style) reliably states
# whether it is an undiscounted principal/face-value cash-flow amount
# (the near-universal ASC 835/470 maturity-schedule convention) rather
# than a GAAP carrying amount. Matched against the concept qname's local
# name (after the namespace prefix), never the label alone — a company
# could phrase the LABEL any number of ways, but the underlying us-gaap
# (or extension) concept name is the authoritative signal for what is
# actually being reported.
CARRYING_VALUE_REJECT_CONCEPT_PATTERN = r"principal|face(?:value|amount)"

# Requirement 3 (accounting policy): exclude any maturity-schedule row
# that is not itself interest-bearing DEBT — a lease liability, purchase
# obligation, interest-payment schedule, or other non-debt commitment
# sometimes appears alongside a genuine debt maturity table (or, rarely,
# is presented in a role that also matches DEBT_MATURITY_ROLE_PATTERN).
# Checked against both the row's own label and its concept qname.
...
def attempt_current_debt_from_maturity_bucket(
    model_xbrl: Any,
    presentation: pd.DataFrame,
    expected_cik: int,
    report_date: str,
    ...
    """
    Only called after attempt_current_debt_zero_inference has already
    raised TargetRowNotFound (the zero-proof failed). Locates the SAME
    earliest maturity-schedule bucket D-017's condition 2 already
    checks, and evaluates it as a candidate current_debt VALUE (not a
    zero-proof). Returns None if no maturity schedule role can be
    uniquely identified at all (nothing to evaluate — the caller's
    existing REVIEW_REQUIRED message is used unchanged). Otherwise
    always returns a dict describing the bucket that was found, with
    `"rejected": False` and a `"value"` only when EVERY evidence check
    (role, non-debt exclusion, GAAP-carrying-amount concept, period
    type, balance type, unique matching fact) passes; `"rejected": True`
    with a `"reason"` string otherwise — never a guess, and the rejected
    case still carries full evidence (bucket label, concept, value if
    fetchable, parent chain) for transparency/audit even though it is
    not used.
    """
    ...
    parent_chain = build_ancestor_chain(
        presentation, role_uri, earliest_row["concept_qname"]
    )

    base_evidence: dict[str, object] = {
        "maturity_role_uri": role_uri,
        "bucket_label": earliest_row["label"],
        "bucket_concept_qname": earliest_row["concept_qname"],
        "bucket_period_type": earliest_row["period_type"],
        "bucket_balance": earliest_row.get("balance"),
        "parent_chain": parent_chain,
    }

    # Requirement 3: exclude lease/purchase-obligation/interest/other
    # non-debt commitments, checked against both label and concept name.
    if re.search(
        NON_DEBT_MATURITY_EXCLUDE_PATTERN, earliest_row["label"], re.IGNORECASE
    ) or re.search(
        NON_DEBT_MATURITY_EXCLUDE_PATTERN,
        earliest_row["concept_qname"],
        re.IGNORECASE,
    ):
        return {
            **base_evidence,
            "rejected": True,
            "reason": (
                f"השורה המוקדמת ביותר ('{earliest_row['label']}') מזוהה "
                "כהתחייבות שאינה חוב נושא-ריבית (חכירה / התחייבות רכש / "
                "תשלום ריבית) — לא ניתן להשתמש בה כ-current_debt."
            ),
        }

    # Requirement 1: reject undiscounted principal/face-value concepts —
    # the near-universal debt-maturity-schedule convention (ASC
    # 835/470), never a GAAP carrying amount.
    concept_local_name = str(earliest_row["concept_qname"]).split(":")[-1]
    ...
    if re.search(
        ...
        return {
            **base_evidence,
            "rejected": True,
            "reason": (
                f"ה-concept של השורה המוקדמת ביותר "
                f"({earliest_row['concept_qname']}) מייצג במפורש סכום "
                "קרן (Principal) לא-מהוון בלוח פירעונות, לא שווי בספרים "
                "(GAAP Carrying Amount) — אסור להשתמש בו כ-current_debt "
                "לפי דרישת המדיניות (לא Face Value / Principal)."
            ),
        }

    if str(earliest_row.get("period_type")) != "instant":
        return {
            **base_evidence,
            "rejected": True,
            "reason": (
                "השורה המוקדמת ביותר אינה בעלת period_type='instant' — "
                "אינה עומדת בסטנדרט הראייתי הנדרש."
            ),
        }

    if str(earliest_row.get("balance", "")) != "credit":
        return {
            **base_evidence,
            "rejected": True,
            "reason": (
                "השורה המוקדמת ביותר אינה בעלת יתרת Credit — אינה עומדת "
                "בסטנדרט הראייתי הנדרש עבור שורת חוב."
            ),
        }
    ...
    if decision["status"] != "PASS":
        return {
            **base_evidence,
            ...
            "reason": (
                "לא ניתן לחלץ ערך מהימן ויחיד עבור השורה המוקדמת ביותר "
                f"('{earliest_row['label']}') — {decision.get('error')}"
            ),
        }

    # Scope guard (critical): this policy targets ONLY the
    # "earliest bucket is NONZERO" root cause by name and by design — a
    # zero-valued earliest bucket means D-017's own condition 2 already
    # succeeds, so if attempt_current_debt_zero_inference still raised
    # TargetRowNotFound, the real unresolved problem is a DIFFERENT
    # condition (3: schedule total doesn't reconcile with long_term_debt,
    # or 4: a contradicting fact elsewhere) — found while regression-
    # testing on Meta, whose zero-inference fails at condition 3 (a
    # genuine ~$174M schedule/balance-sheet discrepancy) while its own
    # earliest bucket is already 0. Silently accepting a $0 bucket value
    # here would mask that unrelated, still-unresolved discrepancy
    # instead of fixing the one thing this policy is scoped to fix.
    # Never triggered for AMZN/GOOGL's actual target scenario, where the
    # bucket is genuinely nonzero.
    if decision["selected_value"] == 0:
        return {
            **base_evidence,
            "rejected": True,
            "reason": (
                "השורה המוקדמת ביותר בלוח הפירעונות היא אפס — מדיניות "
                "זו (D-021) מיועדת אך ורק למקרה שבו השורה המוקדמת ביותר "
                "אינה אפס; היעדר הוכחת current_debt=0 נובע כאן מתנאי "
                "אחר (3 או 4) של D-017 שנותר בלתי-פתור, ואין להשתמש "
                "בבאקט האפס הזה כדי לעקוף אותו."
            ),
        }
    ...
    return {
        **base_evidence,
        "rejected": False,
        "reason": (
            "ה-concept אינו Principal/Face Value, עבר את כל בדיקות "
            "הראיה (Role, שרשרת Ancestor, סוג תקופה, יתרת Credit, ערך "
            "עובדתי יחיד) — מתקבל כ-current_debt."
        ),
        "value": decision["selected_value"],
        "context_id": decision["selected_context_id"],
        "period_start": decision["selected_period_start"],
        "period_end": decision["selected_period_end"],
        "unit": decision["selected_unit"],
        "decimals": decision["selected_decimals"],
    }
...
def engine_child_worker(
    ...
    arelle_load_seconds: float | None = None
    ...
    try:
        ...
        arelle_load_start = time.perf_counter()

        with Session() as session:
            session.run(options)

            arelle_load_seconds = time.perf_counter() - arelle_load_start
            ...
            for metric_name in row_identify_metric_names:
                try:
                    if metric_name == "current_debt":
                    ...
                    elif metric_name == "pretax_income":
                        metric = BUILT_IN_METRICS[metric_name]
                        ...
                        try:
                            ...
                            # Structural fallback (accounting policy
                            # D-020): the label-based search found
                            # nothing unambiguous — try the presentation
                            # row immediately preceding
                            # income_tax_expense within the same role
                            # (see find_pretax_income_by_structural_position).
                            # income_tax_expense is always resolved
                            # first, since row_identify_metric_names is
                            # processed in sorted order and
                            # "income_tax_expense" < "pretax_income".
                            income_tax_expense_rows = target_rows.get(
                                "income_tax_expense"
                            )
                            income_tax_expense_row = (
                                income_tax_expense_rows[0]
                                if income_tax_expense_rows
                                else None
                            )

                            structural_row = (
                                find_pretax_income_by_structural_position(
                                    presentation, income_tax_expense_row
                                )
                            )
                            ...
                            if structural_row is not None:
                                ...
                                raise TargetRowNotFound(
                                    "לא נמצאה אף שורת 'pretax_income' "
                                    "בתוך Statement role ראשי התואם "
                                    "לכללי ה-Role של המדד, וגם לא נמצאה "
                                    "שורה מבנית התואמת (הקודמת ל-"
                                    "income_tax_expense) — מדיניות "
                                    "D-020."
                                )
                    else:
                        metric = BUILT_IN_METRICS[metric_name]
            ...
            for (
                ...
                if metric_name == "current_debt" and (
                    ...
                    try:
                        ...
                        # D-021 (attempted, bounded): the zero-proof
                        # failed — before giving up, check whether the
                        # SAME earliest maturity bucket carries a usable
                        # GAAP-carrying-amount value (never a
                        # face-value/principal amount — see
                        # attempt_current_debt_from_maturity_bucket).
                        # Always evidence-recorded, whether or not it
                        # resolves, for full audit transparency.
                        bucket_result = (
                            attempt_current_debt_from_maturity_bucket(
                                model_xbrl,
                                presentation,
                        ...
                        if bucket_result is not None and not bucket_result.get(
                            "rejected", True
                        ):
                            metric_result["target_concept_qname"] = (
                                bucket_result["bucket_concept_qname"]
                            )
                            metric_result["target_role_uri"] = bucket_result[
                                "maturity_role_uri"
                            ]
                            metric_result["target_role_definition"] = (
                                "current_debt מלוח פירעונות חוב, שווי "
                                "בספרים (D-021, מדיניות מוצעת)"
                            )
                            metric_result["target_label"] = bucket_result[
                                "bucket_label"
                            ]
                            metric_result["selection_tier"] = (
                                "maturity_schedule_bucket_carrying_value"
                            )
                            metric_result["period_type"] = bucket_result[
                                "bucket_period_type"
                            ]
                            metric_result["status"] = "PASS"
                            metric_result["matched_fact_count"] = 1
                            metric_result["filtered_fact_count"] = 1
                            metric_result["distinct_value_count"] = 1
                            metric_result["selected_value"] = bucket_result[
                                "value"
                            ]
                            metric_result["selected_context_id"] = (
                                bucket_result["context_id"]
                            )
                            metric_result["selected_period_start"] = (
                                bucket_result["period_start"]
                            )
                            metric_result["selected_period_end"] = (
                                bucket_result["period_end"]
                            )
                            metric_result["selected_unit"] = bucket_result[
                                "unit"
                            ]
                            metric_result["selected_decimals"] = (
                                bucket_result["decimals"]
                            )
                            metric_result["note"] = bucket_result["reason"]
                            metric_result["components_detail"] = [
                                {
                                    "type": (
                                        "maturity_bucket_carrying_value_"
                                        "evidence"
                                    ),
                                    **bucket_result,
                                }
        ...
        json.dumps(
            {
                ...
                "arelle_load_seconds": arelle_load_seconds,
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v15 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
    ...
    # Post-processing phase timing ("validation_seconds") — everything
    # from here (reading the child's raw per-metric results) through
    # computing custom/derived metrics and writing every output file,
    # distinct from Arelle loading and raw extraction (both measured
    # inside the child process and reported via child_result).
    ...
    final_result = {
        ...
        "arelle_load_seconds": (
            round(child_result["arelle_load_seconds"], 4)
            if child_result.get("arelle_load_seconds") is not None
            else None
        ),
        "extraction_seconds": (
            round(child_result["extraction_seconds"], 4)
            if child_result.get("extraction_seconds") is not None
            else None
        ),
