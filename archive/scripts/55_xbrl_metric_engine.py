...
#
# v9 (this file, 55, copied from 54/v8): implements accounting policy
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_engine_v9_presentation.csv"
        ),
        "row_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v9_row_candidates.csv"
        ),
        "fact_candidates_csv": (
            DATA_DIR / f"{prefix}_engine_v9_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_engine_v9_result.json",
        "arelle_log_file": (
            DATA_DIR / f"{prefix}_engine_v9_arelle_child.log"
        ),
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_engine_v9_orchestration.log"
        ),
    }
...
def run_engine(
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — GENERIC XBRL METRIC ENGINE v9 "
        f"[{', '.join(metric_names)}]",
    )
    log_line(paths["orchestration_log_file"], "=" * 100)
    log_line(
