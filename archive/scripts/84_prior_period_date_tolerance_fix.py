...
TARGET_FILINGS: list[tuple[str, str, str]] = [
    ("MU", "2021-09-02", "0000723125-21-000065"),
    ("MU", "2022-09-01", "0000723125-22-000048"),
    ("MU", "2023-08-31", "0000723125-23-000054"),
    ("MU", "2024-08-29", "0000723125-24-000027"),
]
CONTROL_FILING: tuple[str, str, str] = ("MU", "2025-08-28", "0000723125-25-000028")
...
AFFECTED_METRICS = ["average_invested_capital", "roic"]
...
# D-024: same general policy already used live (see docs/DECISIONS_LOG.md
# and scripts/69+'s PRIOR_PERIOD_DATE_TOLERANCE_DAYS) — a 52/53-week
# fiscal-calendar filer's actual prior-fiscal-year-end date can be a few
# days off from a naive "-1 year, same month/day" calculation. Applied
# ONLY to prior-period lookups (via the date_tolerance_days parameter
# below, default 0 = exact match, unchanged for the current, accession-
# locked report_date). Widening the match window never widens what
# counts as ambiguous: if it lets in more than one genuinely distinct
# value, the existing distinct-value check below still fails closed to
# REVIEW_REQUIRED exactly as before — this mirrors the live engine's own
# documented behavior for the same tolerance.
...
def match_facts_from_warehouse(
    ...
    matching period end/instant date (within `date_tolerance_days` of
    `report_date` — 0 by default, exact match, widened only for prior-
    period requests), correct period type, not nil, numeric value
    present — then decides PASS (exactly one distinct value) vs
    REVIEW_REQUIRED. Evidence about the date match itself (requested
    date, actual matched date(s), day difference, tolerance applied) is
    always included in the returned dict for audit, whether or not a
        ...
        return {
            ...
            "date_evidence": {
                k: decision.get(k) for k in (
                    "requested_date", "matched_date", "matched_dates",
                    "date_diff_days", "date_tolerance_days",
                )
            },
        }
...
def compute_company_year(
    ...
    # D-024: prior-period lookups now use the SAME general
    # PRIOR_PERIOD_DATE_TOLERANCE_DAYS policy already used by the live
    # engine — the current, accession-locked report_date above remains
    # an exact match (date_tolerance_days=0, unchanged).
    cache_prior: dict[str, dict[str, object]] = {}
    ltd_prior = reconstruct_metric(
        connection, accession_number, presentation, "long_term_debt", prior_report_date,
        cache_prior, date_tolerance_days=PRIOR_PERIOD_DATE_TOLERANCE_DAYS,
    )
    cache_prior["long_term_debt"] = ltd_prior
    cd_prior = reconstruct_metric(
        connection, accession_number, presentation, "current_debt", prior_report_date,
        cache_prior, date_tolerance_days=PRIOR_PERIOD_DATE_TOLERANCE_DAYS,
    )
    ...
    cash_prior = reconstruct_metric(
        connection, accession_number, presentation, "cash_and_equivalents", prior_report_date,
        cache_prior, date_tolerance_days=PRIOR_PERIOD_DATE_TOLERANCE_DAYS,
    )
    sti_prior = reconstruct_metric(
        connection, accession_number, presentation, "short_term_investments", prior_report_date,
        cache_prior, date_tolerance_days=PRIOR_PERIOD_DATE_TOLERANCE_DAYS,
    )
    equity_prior = reconstruct_metric(
        connection, accession_number, presentation, "stockholders_equity", prior_report_date,
        cache_prior, date_tolerance_days=PRIOR_PERIOD_DATE_TOLERANCE_DAYS,
    )
    ...
    return {
        ...
        "prior_report_date_requested": prior_report_date,
        ...
        "cash_and_equivalents_prior": cash_prior, "short_term_investments_prior": sti_prior,
        "stockholders_equity_prior": equity_prior,
...
def _compute_roic(result: dict[str, object], ground_truth: dict[str, object]) -> dict[str, object]:
    nopat = ground_truth.get("nopat", {})  # unaffected by this policy — read as-is
    avg_ic = result["average_invested_capital"]
    status = (
        "PASS_MATURITY_BASIS"
        if avg_ic["status"] == "PASS_MATURITY_BASIS" and nopat.get("status") == "PASS"
        else "PASS" if avg_ic["status"] == "PASS" and nopat.get("status") == "PASS"
        else "REVIEW_REQUIRED"
    )
    nopat_value = nopat.get("value") if nopat.get("value") is not None else nopat.get("selected_value")
    value = (
        nopat_value / avg_ic["value"]
        if status in SUCCESSFUL_STATUSES and avg_ic["value"] and avg_ic["value"] > 0
        else None
    )
    return {"status": status, "value": value}


def _print_date_evidence(label: str, metric_result: dict[str, object]) -> None:
    ev = metric_result.get("date_evidence") or {}
    print(
        f"    {label:26s} requested={ev.get('requested_date')} "
        f"matched={ev.get('matched_date') or ev.get('matched_dates')} "
        f"diff_days={ev.get('date_diff_days')} tolerance={ev.get('date_tolerance_days')} "
        f"status={metric_result.get('status')} value={metric_result.get('value')}"
    )
...
def main() -> None:
    ...
    # --- STEP 1: control test (MU 2025-08-28) — must reproduce the
    # existing PASS average_invested_capital/roic before proceeding -----
    print("=== CONTROL TEST: MU 2025-08-28 ===")
    control_start = time.perf_counter()
    ticker, report_date, accession_number = CONTROL_FILING
    control_result = compute_company_year(connection, ticker, report_date, accession_number)
    control_query_seconds = time.perf_counter() - control_start

    control_ground_truth, gt_version = load_latest_ground_truth(ticker, report_date)
    control_result["roic"] = _compute_roic(control_result, control_ground_truth)
    all_results[f"{ticker}_{report_date}"] = control_result

    print(f"  prior_report_date_requested = {control_result['prior_report_date_requested']}")
    _print_date_evidence("current_debt_prior", control_result["current_debt_prior"])
    _print_date_evidence("long_term_debt_prior", control_result["long_term_debt_prior"])
    _print_date_evidence("cash_and_equivalents_prior", control_result["cash_and_equivalents_prior"])
    _print_date_evidence("stockholders_equity_prior", control_result["stockholders_equity_prior"])

    control_pass = True
    for metric_name in AFFECTED_METRICS:
        old = control_ground_truth.get(metric_name, {})
        old_value = old.get("value") if old.get("value") is not None else old.get("selected_value")
        old_status = old.get("status")
        new = control_result[metric_name]
        new_value, new_status = new.get("value"), new.get("status")
        values_match = old_value == new_value or (
            old_value is not None and new_value is not None and abs(old_value - new_value) < 1e-6
        )
        is_match = values_match and old_status == new_status
        if not is_match:
            control_pass = False
        print(f"  {metric_name:24s} old={old_status}/{old_value}  new={new_status}/{new_value}  {'OK' if is_match else 'DIFF'}")

    print(f"  control_query_seconds = {control_query_seconds:.4f}")
    print(f"CONTROL REPRODUCES VERIFIED RESULT: {control_pass}")

    if not control_pass:
        print("\nSTOPPING: control test did not reproduce the verified live-engine "
              "result — per instruction, investigating instead of proceeding to the "
              "4 affected years.")
        connection.close()
        return

    # --- STEP 2: the 4 affected MU years ---------------------------------
    print("\n" + "=" * 100)
    print("STEP 2: 4 affected MU years")
    for ticker, report_date, accession_number in TARGET_FILINGS:
        ...
        query_seconds = time.perf_counter() - filing_start

        ground_truth, gt_version = load_latest_ground_truth(ticker, report_date)
        result["roic"] = _compute_roic(result, ground_truth)
        all_results[f"{ticker}_{report_date}"] = result

        print(f"  prior_report_date_requested = {result['prior_report_date_requested']}")
        _print_date_evidence("current_debt_prior", result["current_debt_prior"])
        _print_date_evidence("long_term_debt_prior", result["long_term_debt_prior"])
        _print_date_evidence("cash_and_equivalents_prior", result["cash_and_equivalents_prior"])
        _print_date_evidence("stockholders_equity_prior", result["stockholders_equity_prior"])
        print(f"  query_seconds={query_seconds:.4f}s  ground_truth_version={gt_version}")
        for metric_name in AFFECTED_METRICS:
    ...
    print("\n" + "=" * 100)
    print("COMPARISON vs. previous ground truth (affected metrics only)")
    conversions: list[str] = []
    still_review_required: list[str] = []

    for key, result in all_results.items():
        if key == f"{CONTROL_FILING[0]}_{CONTROL_FILING[1]}":
            continue
    ...
    print(f"\nConverted to PASS ({len(conversions)}):")
    ...
    output_path = DATA_DIR / "d024_prior_period_tolerance_results.json"
