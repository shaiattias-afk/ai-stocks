"""
Consolidated-cleanup warehouse loader (per user's D-026-follow-up task,
groups 1/3/4) — extends scripts/73/75/78/81/86 (warehouse architecture
approved D-021) to the 11 filings still missing from the warehouse that
are needed for this cleanup:
  - PANW 2025-07-31 (Group 1)
  - CRWD 2022-01-31, 2023-01-31, 2024-01-31, 2025-01-31, 2026-01-31 (Group 3, all 5)
  - META 2020-12-31, 2021-12-31, 2022-12-31, 2023-12-31 (Group 4; 2024-12-31
    already warehoused by scripts/73)
  - NVDA 2020-01-26 (Group 4; 2024-01-28 already warehoused by scripts/73)

Every extraction function below is copied UNCHANGED from scripts/86 (no
ticker- or year-specific parsing/mapping logic anywhere) — only the
module docstring, SCRIPT_NAME, and TARGET_FILINGS list differ. Written
into the SAME warehouse (data/database/xbrl_warehouse_proof.duckdb),
idempotent per accession_number, resumable (already_loaded() skips any
filing that already has a PASS warehouse_runs row from ANY prior
loading script). Each filing is parsed by Arelle exactly once, and the
warehouse is populated before that filing's Arelle session is closed.

This script only builds the raw structured warehouse layer — it does
not compute or write any canonical metric or accounting-policy result.
"""
...
SCRIPT_NAME = "91_build_xbrl_warehouse_groups_1_3_4.py"
EXPECTED_FORM = "10-K"
...
# =============================================================================
# Filing lock loading (identical logic to scripts/69+72/86's
...
# =============================================================================
# Extraction helpers (unchanged from scripts/86)
# =============================================================================
...
# =============================================================================
# DuckDB schema + writing (unchanged from scripts/86)
# =============================================================================
...
def main() -> None:
    ...
    for ticker, report_date in TARGET_FILINGS:
        ...
        print(f"[{ticker} {report_date}] elapsed={filing_elapsed:.1f}s "
              f"total_so_far={time.perf_counter()-task_start:.1f}s")
        print()

    print("=" * 100)
    print("GROUPS 1/3/4 WAREHOUSE LOAD — SUMMARY")
    for summary in summaries:
        print(
            f"  {summary['ticker']:6s} {summary['report_date']:12s} "
            f"accession={summary['accession_number']} "
            f"status={summary['status']} "
            f"elapsed={summary.get('filing_elapsed_seconds')}"
        )
    print(f"total_elapsed_seconds = {time.perf_counter() - task_start:.3f}")
    print("=" * 100)
