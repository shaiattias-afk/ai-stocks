"""
D-029 — warehouse loader for the 3 newly-locked prior-fiscal-year
filings needed to close the first-year Average Invested Capital gap
identified in D-028: AMZN FY2020, GOOGL FY2020, META FY2019. Extends
scripts/73/75/78/86/91 (every prior loading script preserved unchanged;
every extraction function below is copied UNCHANGED — no ticker- or
year-specific parsing/mapping logic anywhere).

These 3 filings were freshly locked (form=10-K, exact reportDate,
accessionNumber, filingDate, primaryDocument — via
scripts/36b_download_accession_locked_filing.py, unmodified, called
with --ticker/--report-date for AMZN 2020-12-31, GOOGL 2020-12-31, META
2019-12-31) and saved to the existing sec_filings_locked location
before this script ran. Parsed with Arelle exactly once each, written
into the SAME warehouse (data/database/xbrl_warehouse_proof.duckdb),
idempotent per accession_number.

Used ONLY to compute invested_capital for the prior fiscal year itself
(via a later script) — no later filing's facts are read when resolving
these, and no fact from AMZN/GOOGL 2021 or META 2020 is used when
resolving these prior years, preserving point-in-time correctness.
"""
...
SCRIPT_NAME = "97_build_xbrl_warehouse_prior_year_gap.py"
EXPECTED_FORM = "10-K"
...
def main() -> None:
    ...
    print("=" * 100)
    print("PRIOR-YEAR-GAP WAREHOUSE LOAD — SUMMARY")
