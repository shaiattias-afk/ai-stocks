"""
D-034 — warehouse loader for the final remaining "first fiscal year in
dataset" gap identified in D-033: NVDA FY2019 (report date 2019-01-27),
needed ONLY as the prior fiscal year for NVDA FY2020's
average_invested_capital/roic. Extends scripts/73/75/78/86/91/97/100
(every prior loading script preserved unchanged; every extraction
function below is copied UNCHANGED — no ticker- or year-specific
parsing/mapping logic anywhere).

This filing was freshly locked (form=10-K, exact reportDate,
accessionNumber, filingDate, primaryDocument — via
scripts/36b_download_accession_locked_filing.py, unmodified, called
with --ticker NVDA --report-date 2019-01-27) and saved to the existing
sec_filings_locked location before this script ran. Parsed with Arelle
exactly once, written into the SAME warehouse
(data/database/xbrl_warehouse_proof.duckdb), idempotent per
accession_number.

Used ONLY to compute invested_capital for NVDA's FY2019 itself (via a
later script) — no fact from NVDA FY2020 is read when resolving this,
preserving point-in-time correctness. No other company is touched.
"""
...
SCRIPT_NAME = "104_build_xbrl_warehouse_nvda_first_year_gap.py"
EXPECTED_FORM = "10-K"
...
# NVDA's FY2019 10-K (filed 2019-02-21) predates NVDA's Inline XBRL
# adoption — SEC's own "primaryDocument" field for this filing is the
# human-readable HTML (nvda-2019x10k.htm), which carries NO embedded
# ix: facts at all (confirmed empirically: a first load attempt via
# this file produced 0 facts/0 contexts/0 units — a real, diagnosable
# result, not a silent failure). This filing instead ships a SEPARATE,
# traditional (non-inline) XBRL instance document,
# nvda-20190127.xml, alongside the HTML — the same accession, same
# locked package, already saved to disk; this is not a different or
# later filing, just the correct entry point for Arelle within the
# SAME already-locked accession. Confirmed present in the same locked
# directory before use.
ENTRYPOINT_OVERRIDE_BY_ACCESSION: dict[str, str] = {
    "0001045810-19-000023": "nvda-20190127.xml",
}
...
def load_locked_filing(ticker: str, report_date: str) -> dict[str, Any]:
    ...
    override_filename = ENTRYPOINT_OVERRIDE_BY_ACCESSION.get(accession_number)
    if override_filename:
        primary_document_path = primary_document_path.parent / override_filename
        primary_document_name = override_filename
...
def main() -> None:
    ...
    print("=" * 100)
    print("NVDA FIRST-YEAR-GAP WAREHOUSE LOAD — SUMMARY")
