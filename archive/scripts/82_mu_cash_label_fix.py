...
AFFECTED_METRICS = [
    "cash_and_equivalents", "adjusted_net_debt",
...
def main() -> None:
    ...
    all_filings = TARGET_FILINGS + [CONTROL_FILING]

    for ticker, report_date, accession_number in all_filings:
    ...
    # --- control check: MU 2025-08-28 must be byte-identical (status +
    # value) to its pre-existing ground truth for every AFFECTED_METRIC
    # — this filing's label already matched the OLD pattern, so the
    # broadened regex must change nothing about it.
    print("\n" + "=" * 100)
    print("CONTROL CHECK — MU 2025-08-28 must be unchanged")
    control_key = f"{CONTROL_FILING[0]}_{CONTROL_FILING[1]}"
    control_result = all_results[control_key]
    control_ground_truth, _ = load_latest_ground_truth(CONTROL_FILING[0], CONTROL_FILING[1])
    control_clean = True
    for metric_name in AFFECTED_METRICS:
        ...
        values_match = old_value == new_value or (
            old_value is not None and new_value is not None and abs(old_value - new_value) < 1
        )
    ...
    print(f"CONTROL BYTE-IDENTICAL: {control_clean}")

    output_path = DATA_DIR / "d023_mu_cash_fix_results.json"
