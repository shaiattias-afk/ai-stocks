...
# in scripts/37c and scripts/37d, and on Microsoft in scripts/38.
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generic, ticker-agnostic extraction of the top-level "
            "revenue fact from a locked 10-K, using Arelle statement-"
            "first presentation structure only (no per-company tag)."
        )
    )
...
def output_paths(ticker: str, report_date: str) -> dict[str, Path]:
    ...
    return {
        "presentation_csv": (
            DATA_DIR / f"{prefix}_v2_arelle_presentation.csv"
        ),
        "candidates_csv": (
            DATA_DIR / f"{prefix}_v2_revenue_fact_candidates.csv"
        ),
        "result_file": DATA_DIR / f"{prefix}_v2_revenue_fact_result.json",
        "arelle_log_file": DATA_DIR / f"{prefix}_v2_arelle_child.log",
        "orchestration_log_file": (
            DATA_DIR / f"{prefix}_v2_orchestration.log"
        ),
    }
...
class TargetRowNotFound(Exception):
    ...
    one unambiguous top-level revenue row. Distinguished from other
    exceptions so the caller can report REVIEW_REQUIRED (insufficient
    evidence) instead of FAIL (execution error).
    """


def select_target_row(presentation: pd.DataFrame) -> dict[str, str]:
    """
    Selects the single row representing the top-level revenue line on the
    primary income statement, using structure and label only:
    ...
    - a non-abstract row whose label is exactly "Revenue", "Revenues",
      "Total revenue" or "Total revenues" (case-insensitive) — covering
      both the single-line-item convention (e.g. Microsoft: "Revenue")
      and the summed-subtotal convention (e.g. Oracle: "Total revenues").
    No concept name is hard-coded for any specific issuer. If this does
    not resolve to exactly one row, raises TargetRowNotFound.
    """
    ...
    is_revenue_label = presentation["label"].str.match(
        r"(?i)^\s*(total\s+)?revenues?\s*$",
        na=False,
    )
    ...
    if len(candidates) != 1:
        raise TargetRowNotFound(
            "לא נמצאה שורת Presentation יחידה וחד-משמעית של הכנסות "
            "(Revenue/Revenues/Total revenue/Total revenues) בתוך "
            "Statement role של Income/Operations.\n"
...
def run_extraction(ticker: str, report_date: str) -> dict[str, object]:
    ...
    log_line(
        ...
        f"{ticker.upper()} {report_date} — REVENUE FACT EXTRACTION v2 "
